...
const NewMessageToast: React.FC<NewMessageToastProps> = ({ message, toastId }) => {
  return (
    <motion.div
      ...
      className="relative flex items-center gap-4 w-full max-w-sm p-4 bg-gradient-to-br from-blue-400 to-cyan-500 text-white rounded-2xl shadow-2xl border-2 border-white/50 overflow-hidden"
    >
      <div className="absolute -top-4 -left-4 w-16 h-16 bg-white/20 rounded-full animate-pulse" />
      <div className="shrink-0 z-10">
        <PartyPopper className="w-8 h-8 text-cyan-200" />
      </div>
